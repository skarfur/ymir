...
// Ports payroll.gs's clockOut_ — any authenticated session.
...
Deno.serve(async (req: Request) => {
  ...
  const dur = Math.round((new Date(now).getTime() - new Date(lastIn.ts).getTime()) / 60000);
  const { error } = await admin.from("time_clock").insert({
    id: crypto.randomUUID(), employee_id: employeeId, type: "out", ts: now,
    source: body?.source || "staff", original_ts: null,
    note: body?.note || "", period_key: periodKey(), duration_minutes: dur,
  });
  if (error) return json({ error: "clockOut failed: " + error.message }, 500);

  return json({ clocked: "out", timestamp: now, durationMinutes: dur });
});
