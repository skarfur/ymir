...
// Ports share.gs's revokeShareToken_ — any authenticated session, but
// only the token's own owner (memberKennitala match) may revoke it.
...
Deno.serve(async (req: Request) => {
  ...
  const { error } = await admin.from("share_tokens").update({ revoked_at: new Date().toISOString() }).eq("id", tokenId);
  if (error) return json({ error: "revokeShareToken failed: " + error.message }, 500);
