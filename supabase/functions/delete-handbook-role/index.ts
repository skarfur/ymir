...
// Ports handbook.gs's deleteHandbookRole_ — admin only. Soft-delete
// leaves children's parentId intact so the admin can re-parent.
...
Deno.serve(async (req: Request) => {
  ...
  const ok = await softDeleteConfigListItem(admin, "handbookRoles", id);
  if (!ok) return json({ error: "Role not found" }, 404);
