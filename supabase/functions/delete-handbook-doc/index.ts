...
// Ports handbook.gs's deleteHandbookDoc_ — admin only. Soft-deletes the
// config-list entry. Trashing the backing Drive file
// (DriveApp.getFileById(...).setTrashed(true)) is deliberately not
// ported — Drive integration is out of scope everywhere this migration.
...
Deno.serve(async (req: Request) => {
  ...
  const ok = await softDeleteConfigListItem(admin, "handbookDocs", id);
  if (!ok) return json({ error: "Doc not found" }, 404);
