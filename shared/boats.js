...
function renderBoatCard(boat, opts) {
  ...
  const badgeMap = {
    ...
    out:     { text:s("fleet.badgeOut"),      style:"color:var(--accent);border-color:var(--accent)55;background:var(--accent)11" },
  ...
  if (priv && boat.ownerName) {
    ownerLine = `<div style="font-size:10px;color:var(--accent);margin-top:4px">${_besc(s("fleet.ownedBy",{name:boat.ownerName}))}</div>`;
  }
  let charterLine = "";
  if (activeRes) {
    charterLine = `<div style="font-size:10px;color:var(--accent);margin-top:4px">`
  ...
  if (controlled && !userCanAccess && !opts.staffView) {
    ...
    ownerBadge = `<span style="font-size:9px;letter-spacing:.8px;padding:2px 7px;border-radius:10px;border:1px solid;color:var(--accent);border-color:var(--accent)55;background:var(--accent)11;margin-left:4px">${_besc(s("fleet.badgeChartered"))}</span>`;
...
function renderCheckoutCard(co, opts) {
  ...
  if (!staffView) {
    ...
    else              topBadge = `<span style="font-size:9px;letter-spacing:.8px;padding:2px 7px;border-radius:10px;border:1px solid;color:var(--accent);border-color:var(--accent)55;background:var(--accent)11">${_besc(s("fleet.badgeOut"))}</span>`;
  } else if (overdue) {
  ...
  if (staffView) {
    ...
    if (isMinor && co.guardianName) {
      contactHtml = `<div style="font-size:11px;color:var(--muted);background:var(--card);border:1px solid var(--accent)44;border-radius:6px;padding:7px 10px;margin-top:8px;display:flex;align-items:center;gap:8px">`
                  ...
                  + `${co.guardianPhone?`<a href="tel:${_besc(co.guardianPhone)}" style="color:var(--accent);text-decoration:none">${_besc(co.guardianPhone)}</a>`:""}`
                  + `</div>`;
      ...
      contactHtml = `<div style="font-size:11px;color:var(--muted);background:var(--card);border:1px solid var(--border);border-radius:6px;padding:7px 10px;margin-top:8px;display:flex;align-items:center;gap:8px">`
                  + `<span>=</span><a href="tel:${_besc(co.memberPhone)}" style="color:var(--accent);text-decoration:none">${_besc(co.memberPhone)}</a>`
                  + `</div>`;
    }
  }
